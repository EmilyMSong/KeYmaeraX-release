...
import edu.cmu.cs.ls.keymaerax.pt.TermProvable.getNewBranches
...
case class ElidingProvable(provable: Provable, steps: Int, defs: Declaration) extends ProvableSig {
  ...
  override def proveArithmetic(t: QETool, f: Formula): ProvableSig =
    ElidingProvable.proveArithmetic(t,f)

  override def proveArithmeticLemma(t: QETool, f: Formula): Lemma =
    ElidingProvable.proveArithmeticLemma(t,f)
...
object TermProvable {
  def getNewBranches(rule: Rule): Int = {
    rule match {
      case Close(ante,succ)        => 0 //-1
      case CoHide2(ante, succ)     => 0
      case CutRight(fml, succ)     => 1
      case ImplyRight(succ)        => 0
      case AndRight(succ)          => 1
      case CoHideRight(succ)       => 0
      case CommuteEquivRight(succ) => 0
      case EquivifyRight(succ)     => 0
      case EquivRight(succ)        => 1
      case NotRight(succ)          => 0
      case CloseTrue(succ)         => 0 //-1
      case HideRight(succ)         => 0
      case OrRight(succ)           => 0

      case OrLeft(ante)      => 1
      case AndLeft(ante)     => 0
      case HideLeft(ante)    => 0
      case CutLeft(fml,ante) => 1
      case ImplyLeft(ante)   => 1
      case NotLeft(ante)     => 0
      case EquivLeft(ante)   => 1
      case CloseFalse(ante)  => 0 //-1

      case BoundRenaming(what, repl, seq: SeqPos) => 0
      case UniformRenaming(what, repl)            => 0
      case Skolemize(seq: SeqPos)                 => 0
      case Cut(fml)                               => 1
...
case class TermProvable(provable: ProvableSig, pt: ProofTerm, defs: Declaration) extends ProvableSig with Logging {
  ...
  override def apply(rule: Rule, subgoal: Subgoal): ProvableSig = {
    val rp = provable(rule, subgoal)
    TermProvable(rp, RuleApplication(pt, rule, subgoal, provable.subgoals.length), defs)
    //    TermProvable(provable(rule, subgoal), RuleApplication(pt, rule, subgoal, subgoal), defs) //HERE
  }
  ...
  override def apply(subderivation: ProvableSig, subgoal: Subgoal): ProvableSig = {
    ...
    subderivation match {
      case TermProvable(subProvable, subPT, _) =>
        val thePt = Sub(pt, subPT, subgoal, this.provable.subgoals.length) //HERE
      ...
      case subprovable: ProvableSig => {
        ...
        if (coreAxiom.isDefined) {
          ...
          val term = Sub(pt, AxiomTerm(derivedAxiom.get.codeName), subgoal, this.provable.subgoals.length) //HERE
        ...
        //And ditto for rules.
          ...
          val term = Sub(pt, RuleTerm(derivedRule.get.codeName), subgoal, this.provable.subgoals.length) //HERE
